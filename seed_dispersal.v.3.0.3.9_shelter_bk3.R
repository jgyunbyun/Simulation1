...
#}
if (di<7.0 & tree.h2<tree.h &  tree.d2>50){
  tree.Nr6<-rbind(tree.Nr6,tree.Nr2)
}
if (di<3.0 & tree.h2<tree.h & tree.d2<40 ) {
  tree.Nr5<-rbind(tree.Nr5,tree.Nr2)
}
...
######################
big.tree<-subset(big.tree1,V3>=2 & V3<7) #from 7 to 5
#conser.tree<-subset(big.tree1,V3>=10 & V3<40)
#nrow(big.tree)
a=15.589*10^-3
b=0.01696*10^-3
...
#tree.nu1<-setdiff(tree.nu0,tree.Nr4)
#tree.nu2<-setdiff(tree.nu1,tree.Nr5)
tree.nu2<-setdiff(tree.nu,tree.Nr6)
#tree.nu3<-c(tree.nu2,conser.tree$V1)
#tree.nu2<-tree.nu3[!duplicated(tree.nu3)]
tree.nu2<-setdiff(tree.nu2,tree.Nr5)
tree.nu2<-setdiff(tree.nu2,tree.Nr4)
#write table
