...
a2.1<-subset(a2,KKL<mean(a2$KKL)*2 | Nr<100)
a3<-rbind(a2.1,a1.1)
} else 
  #if (nrow(a2)>50) {
a1.1<-subset(a1,KKL<mean(a1$KKL)*1.4 | Nr<100)
a2.1<-subset(a2,KKL<mean(a2$KKL)*1.8 | Nr<100)
a3<-rbind(a2.1,a1.1)
#} else a3<-a0
...
mean.ci3<-mean.ci*2
min.ci<-min(treedata.sm$KKL)
min.ci2<-max(treedata.sm$KKL)*1.5
...
#<7cm control
#if (nrow(treedata.s20)<40) {
treedata3<-subset(treedata.sm, KKL>min.ci & KKL<min.ci2 | KKL>max.ci2) # KKL>max.ci & KKL<max.ci2|
...
#if (nrow(test)<150) {
#result<-result
#} else result<-result[!(result$BHD>=3 & result$BHD<4),]
...
big.tree<-subset(treedata, Per==1 &BHD>80)
...
f.sm<-subset(result,BHD>4 & BHD<6)
#nrow(f.count)
f.smci<-mean(f.sm$KKL)*1.0

if (nrow(f.count)>=60 & nrow(f.count)<100) {
  f.smsub<-subset(f.sm,KKL<f.smci)
} else if (nrow(f.count)>=100) {
...
#nrow(result0.1)
test.no<-subset(result0.1,BHD>7.0 & BHD<20)
test.no2<-subset(result0.1,BHD>10 & BHD<30)
if (nrow(test.no)>70) {
  result0.1<-result0.1[!(result0.1$BHD<7),]
} else if (nrow(test.no2)>60) {
  result0.1<-result0.1[!(result0.1$BHD<6.5),]
}
...
ini.treedata <- read.delim(paste('c:/SILVA/',site,'/',site,'_',sta.per,'02.slv',sep=''), header=FALSE,skip=2)
ini.result2.40<-subset(ini.treedata,V3>=40)
ini.num<-nrow(ini.result2.40)

## First
#result2.40<-subset(result2,BHD>=40) # or >=40cm?
#vol=with(result2,(a+b*BHD*h^2+d*BHD^3)/0.15*0.6)
treedata.p <- read.delim(paste0(fd,'/',site,'_list_',sta.per), header=TRUE)
treedata1.p<-subset(treedata.p,Per==1)

past.ztr <- read.delim(paste0(fd,'/',site,'_',sta.per,'.ztr'), header=FALSE) #sta.per
ztr.p<-c()



for (i in 1:nrow(past.ztr)) {
  tem.p<-subset(treedata1,Nr==past.ztr[i,])
  ztr.p<-rbind(ztr.p,tem.p)
  
} 
ztr.p<-ztr.p[!(ztr.p$BHD<40),]
ztr.num<-(round(nrow(ztr.p)*0.4,digits=0))
result2<-result2[!(duplicated(result2$Nr)),]
if (period-sta.per==10) {
 
    #result2.40<-subset(result2,BHD>=40)
    result2<-result2[!duplicated(result2$Nr),]
    #test<-result2[(result2$BHD>=40),]
    result2<-result2[!(result2$BHD>=40),]
    ...
    result2<-rbind(result2,ztr)
  
  for (i in 1:ztr.num) {
    #result2.40<-subset(result2,BHD>=40)
    test<-result2[(result2$BHD==min(ztr.p$BHD)),]
    if (nrow(test)>=2) {
      #test2<-result2[(result2$h==max(test$h)) & result2$BHD==53.0,]
      test2<-result2[(result2$h==max(test$h) & result2$BHD==min(ztr.p$BHD)),]
      
    } else test2<-test
    
    ztr.p<-ztr.p[!(ztr.p$Nr==test2$Nr),]
    result2<-result2[!(result2$Nr==test2$Nr),]
  } 
...
ztr.p<-ztr.p[!(ztr.p$BHD<40),]
ztr.num<-(round(nrow(ztr.p)*1.0,digits=0)) #0.7
result2<-result2[!(duplicated(result2$Nr)),]
if (period-sta.per==0) {
  for (i in 1:ztr.num) {
    #result2.40<-subset(result2,BHD>=40)
