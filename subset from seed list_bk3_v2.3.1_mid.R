...
if (nrow(a2)<30) {
a1.1<-subset(a1,KKL<mean(a1$KKL)*0.8 | Nr<100)#*0.5)
a2.1<-subset(a2,KKL<mean(a2$KKL)*2 | Nr<100)
a3<-rbind(a2.1,a1.1)
} else 
  #if (nrow(a2)>50) {
a1.1<-subset(a1,KKL<mean(a1$KKL)*0.8 | Nr<100)
a2.1<-subset(a2,KKL<mean(a2$KKL)*2 | Nr<100)
a3<-rbind(a2.1,a1.1)
#} else a3<-a0
...
test1<-subset(result,BHD>=5.5 & BHD <7)
#nrow(result)
if (nrow(test)<60 & nrow(treedata.gr20)<40) {
  result<-result
} else result<-result[!(result$BHD>=5.0 & result$BHD<10 & result$KKL>mean(test1$KKL)),]

test1<-subset(result,BHD>=5.5 & BHD <7)
if (nrow(test1)>50) {
result<-result[!(result$BHD>=5.0 & result$BHD<10 & result$KKL>mean(test1$KKL)),]
}
a=15.589*10^-3
b=0.01696*10^-3
c=0.01883*10^-3
...
if (sum(vol)>500) {
  result.a<-subset(result,BHD>10) #from 50 to 30
  result.b<-subset(result,BHD<=10) #from 50 to 30
  #result.a<-result.a[!(result.a$KKL==max(result.a$KKL)),]
  result.b<-result.b[!(result.b$KKL==max(result.b$KKL)),]
  result<-rbind(result.a,result.b)
}
...
f.sm<-subset(result,BHD>=5.5 & BHD<8)
#nrow(f.count)
f.smci<-mean(f.sm$KKL)*2
...
ini.treedata <- read.delim(paste('c:/SILVA/',site,'/',site,'_','002','.slv',sep=''), header=FALSE,skip=2)
ini.result2.50<-subset(ini.treedata,BHD>=50)
#ini.num<-nrow(ini.result2.50)-3
## First
#1.cut mature trees
treedata.p <- read.delim(paste0(fd,'/',site,'_list_',sta.per), header=TRUE)
treedata1.p<-subset(treedata.p,Per==1)

result2.40<-subset(treedata1.p,BHD>15 & BHD<45)
treep.num<-(round(nrow(result2.40)*0.5,digits=0))
result2<-result2[!(duplicated(result2$Nr)),]

if (period-sta.per==0) {
  for (i in 1:treep.num) {
    test<-result2.40[result2.40$BHD==max(result2.40$BHD),]
    if (nrow(test)>=2) {
      #test2<-result2[(result2$h==max(test$h)) & result2$BHD==53.0,]
      test2<-result2.40[result2.40$h==max(test$h),]
    } else test2<-test
    result2.40<-result2.40[!(result2.40$Nr==test2$Nr),]
...
#result2.40<-subset(treedata1.p,BHD>=20 & BHD<40)
#treep.num<-(round(nrow(result2.40)*0.5,digits=0))
...
if (period-sta.per==2) {
  for (i in 1:treep.num) {
    test<-result2.40[result2.40$BHD==min(result2.40$BHD),]
    if (nrow(test)>=2) {
      #test2<-result2[(result2$h==max(test$h)) & result2$BHD==53.0,]
      test2<-result2.40[result2.40$Nr==max(test$Nr),]
    } else test2<-test
    #test2.1<-rbind(test2.1,test2)
    result2.40<-result2.40[!(result2.40$Nr==test2$Nr),]
    result2<-result2[!(result2$Nr==test2$Nr),]
  }
}
...
#1.cut big trees
if (period-sta.per==4) {
  result2<-result2[!(result2$BHD>45),]
